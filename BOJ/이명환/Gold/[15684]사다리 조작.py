...
#
# horizontal_lines = [[0 for i in range(n+1)] for j in range(m+1)]
# maximum_lines = [0 for _ in range(n+1)]
#
# for i in range(m):
...
# hori_cnt = 0
#
# def checkLine(horizontal_lines):
#     global vaild_hori
#     for i in range(1,n+1):
#         if not checkDFS(1,i,horizontal_lines,i):
#             return
#
#     vaild_hori = True
#     return
#
# def checkDFS(a,b,horizontal_lines,target):
#     if a == n+1:
#         if b == target:
#             return True
#         else:
#             return False
#     if b-1>=1 and horizontal_lines[a][b-1]:
#         checkDFS(a+1,b-1,horizontal_lines,target)
#     elif horizontal_lines[a][b]:
#         checkDFS(a+1,b+1,horizontal_lines,target)
#     else:
#         checkDFS(a+1,b,horizontal_lines,target)
#
#
# def makeLine(a,b,hori_cnt,horizontal_lines):
#     horizontal_lines_copy = copy.deepcopy(horizontal_lines)
#     if a== m and b == n:
#         print(horizontal_lines_copy)
#         checkLine(horizontal_lines_copy)
#         return
#     elif b == n:
#         makeLine(a+1,1,hori_cnt,horizontal_lines_copy)
#     else:
#         if maximum_lines[b] <= h:
#             makeLine(a,b+1,hori_cnt,horizontal_lines_copy)
#             if hori_cnt and not horizontal_lines_copy[a][b] and not horizontal_lines_copy[a][b-1] and not horizontal_lines_copy[a][b+1]:
#                 horizontal_lines_copy[a][b] = 1
#                 maximum_lines[b]+=1
#                 makeLine(a,b+1,hori_cnt-1,horizontal_lines_copy)
#         else:
#             makeLine(a+1,1,hori_cnt,horizontal_lines_copy)
#
# while(True):
#     makeLine(1,1,hori_cnt,horizontal_lines)
#     if vaild_hori:
#         break
#     hori_cnt+=1
#     print(hori_cnt)
...
#너무 복잡하게 코드를 짜서 큰일이났다.. 그리고 나는 가로선을 체크하는 것을 복사해서 사용했는데 그렇게 하면 안되는 것이였다..
#가로선 체크를 그떄그떄 풀어주면서 사용하면 됐었다.